...
namespace Swashbuckle.AspNetCore.SwaggerGen
{
    public class XmlCommentsSchemaFilter : ISchemaFilter
    {
        ...
        private void ApplyMemberTags(OpenApiSchema schema, SchemaFilterContext context)
        {
            ...
            if (exampleNode != null)
            {
                var exampleAsJson = $"\"{exampleNode.InnerXml.Trim().ToString()}\"";// schema.Type=="string"//(schema.ResolveType(context.SchemaRepository) == "string") && !exampleNode.Value.Equals("null")
                    //? $"\"{exampleNode.InnerXml.Trim().ToString()}\""
